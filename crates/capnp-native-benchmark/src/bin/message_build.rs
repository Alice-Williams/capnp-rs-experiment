...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mode = args
        .next()
        .ok_or("usage: message_build prepared|fresh direct|far PASSES")?;
    ...
    if args.next().is_some()
        ...
        || !matches!(shape.as_str(), "direct" | "far")
        || passes == 0
    {
        return Err("expected prepared|fresh, direct|far, and positive PASSES".into());
    }
    ...
    for pass in 0..passes {
        ...
        let fingerprint = if mode == "prepared" {
            prepared_iteration(far, first, second)
        } else {
            fresh_iteration(far, first, second)?
        };
...
#[inline(never)]
fn prepared_iteration(far: bool, first: u64, second: u64) -> u64 {
