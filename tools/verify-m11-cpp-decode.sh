...
decoded=$(mktemp)
expected=$(mktemp)
trap 'rm -f -- "$flat" "$decoded" "$expected"' EXIT

cd -- "$repo_root"
cargo run -q -p capnp-message --example m11_builder_fixture >"$flat"
"$oracle" decode --flat conformance/schemas/builder-fixture.capnp BuilderFixture \
    <"$flat" >"$decoded"
...
printf 'm11-cpp-decode-ok  %s bytes\n' "$(wc -c <"$flat")"
