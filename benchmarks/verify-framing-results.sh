...
awk -F '\t' -v expected="$expected_results" -v cases="$expected_cases" '
  ...
  $2 != "parse" && $2 != "encode" && $2 != "encode-prepared" { exit 1 }
  $3 != 1 && $3 != 2 && $3 != 64 { exit 1 }
...
awk -F '\t' -v expected="$expected_summary" 'NR == 1 { next } NF != 9 || $4 != 50000 || $5 != 9 || $6 <= 0 || $7 <= 0 || $8 <= 0 || $9 <= 0 { exit 1 } END { if (NR != expected) exit 1 }' "$result_dir/summary.tsv"
