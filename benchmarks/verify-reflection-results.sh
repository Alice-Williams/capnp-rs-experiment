...
if ((expected_cases < 4 || expected_cases > 14)); then
    printf 'expected case count must be from 4 through 14\n' >&2
    exit 2
fi
...
awk -F '\t' -v passes="$expected_passes" -v cases="$expected_cases" -v expected="$expected_raw_lines" '
  ...
  $2 != "schema-name" && $2 != "schema-index" &&
      ...
      !(cases >= 11 && $2 == "dynamic-nested-list") &&
      !(cases >= 12 && $2 == "dynamic-enum") &&
      !(cases >= 13 && $2 == "dynamic-default") &&
      !(cases >= 14 && $2 == "dynamic-union-active") { exit 1 }
