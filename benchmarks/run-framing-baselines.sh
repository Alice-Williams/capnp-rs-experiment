...
runs=${BENCH_RUNS:-9}
passes=${FRAMING_BENCH_PASSES:-50000}
...
cpp_build="$cpp_root/framing-benchmark"
mkdir -p -- "$cpp_build"
clang++ -std=c++23 -O3 -DNDEBUG -pthread \
    -I"$cpp_root/install/include" "$repo_root/benchmarks/framing/cpp/main.c++" \
    -L"$cpp_root/install/lib" -lcapnp -lkj -o "$cpp_build/cpp-framing-benchmark"
cargo build --locked --manifest-path "$repo_root/Cargo.toml" \
    --package capnp-io --example framing_benchmark --release >/dev/null

cpp_benchmark="$cpp_build/cpp-framing-benchmark"
native_benchmark="$repo_root/target/release/examples/framing_benchmark"
...
mkdir -p -- "$output"
{
    ...
    printf 'cpp_primitive=capnp::FlatArrayMessageReader and capnp::messageToFlatArray\n'
    printf 'cpp_stream_read=source-derived contiguous raw-frame reader matching capnp_io::read_frame\n'
    printf 'cpp_stream_write=capnp::writeMessage\n'
    ...
    printf 'timer=steady monotonic clock inside each binary around framing operations\n'
} > "$output/metadata.txt"
...
workloads=(
    ...
    'parse parse 2'
    'parse parse 64'
    'parse-noalloc parse-noalloc 1'
    'parse-noalloc parse-noalloc 2'
    'parse-noalloc parse-noalloc 64'
    'encode encode 1'
    'encode encode 2'
    'encode encode 64'
    ...
    'stream-read-reuse stream-read-reuse 1'
    'stream-read-reuse stream-read-reuse 2'
    'stream-read-reuse stream-read-reuse 64'
...
python3 "$repo_root/tools/summarize-framing-benchmarks.py" \
...
printf 'metadata=%s\nraw=%s\nsummary=%s\ncomparison=%s\n' \
    "$output/metadata.txt" "$output/results.tsv" "$output/summary.tsv" "$output/comparison.tsv"
