...
result_dir=${1:-"$repo_root/benchmarks/results/2026-09-02-native-cpp-g-drive-docker"}
...
test "$(wc -l < "$result_dir/results.tsv")" -eq 111
test "$(wc -l < "$result_dir/summary.tsv")" -eq 11
test "$(wc -l < "$result_dir/comparison.tsv")" -eq 6

awk -F '\t' '
    NR == 1 {
        if ($0 != "implementation\tcase\tmode\tscratch\tcompression\titerations\trun\telapsed_ns\toutput_bytes") exit 1;
        next;
    }
    NF != 9 || $6 <= 0 || $8 <= 0 || $9 < 0 { exit 1 }
    $1 != "cpp" && $1 != "native" { exit 1 }
    END { if (NR != 111) exit 1 }
' "$result_dir/results.tsv"

awk -F '\t' '
    NR == 1 { next }
    NF != 11 || $6 <= 0 || $7 != 9 || $8 <= 0 || $9 <= 0 || $10 <= 0 || $11 <= 0 { exit 1 }
    END { if (NR != 11) exit 1 }
' "$result_dir/summary.tsv"

awk -F '\t' '
    NR == 1 { next }
    NF != 8 || $5 <= 0 || $6 <= 0 || $7 <= 0 || $8 <= 0 { exit 1 }
    END { if (NR != 6) exit 1 }
