...
grep -Fx "parallel_builder_sha256=$(git -C "$repo_root" show "$provenance_commit:crates/capnp-message/src/parallel_builder.rs" | sha256sum | cut -d ' ' -f1)" \
    "$result_dir/metadata.txt"
grep -Fx "benchmark_example_sha256=$(git -C "$repo_root" show "$provenance_commit:crates/capnp-message/examples/parallel_build.rs" | sha256sum | cut -d ' ' -f1)" \
    "$result_dir/metadata.txt"
