...
grep -Fx "parallel_builder_sha256=$(sha256sum "$repo_root/crates/capnp-message/src/parallel_builder.rs" | cut -d ' ' -f1)" \
    "$result_dir/metadata.txt"
grep -Fx "benchmark_example_sha256=$(sha256sum "$repo_root/crates/capnp-message/examples/parallel_build.rs" | cut -d ' ' -f1)" \
    "$result_dir/metadata.txt"
