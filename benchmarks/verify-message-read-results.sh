...
awk -F '\t' -v expected="$expected_results" -v passes="$expected_passes" '
  ...
  $2 != "framing" && $2 != "root" { exit 1 }
  $3 != 1 && $3 != 2 && $3 != 64 { exit 1 }
  !($2 FS $3 in checksum) { checksum[$2 FS $3] = $7; next }
  $7 != checksum[$2 FS $3] { exit 1 }
  END { if (NR != expected || length(checksum) != 6) exit 1 }
' "$result_dir/results.tsv"
awk -F '\t' 'NR == 1 { next } NF != 9 || $6 <= 0 || $7 <= 0 || $8 <= 0 || $9 <= 0 { exit 1 } END { if (NR != 13) exit 1 }' "$result_dir/summary.tsv"
